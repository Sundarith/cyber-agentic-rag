...
**MITRE knowledge graphs.** The MITRE ATT&CK framework [(Strom et al., 2018)](#ref-strom-2018) catalogs adversary tactics, techniques, and procedures. CAPEC enumerates common attack patterns, and the CWE corpus enumerates weakness categories. These three vocabularies are explicitly cross-referenced. Our pipeline exploits these structured bridges as authoritative graph edges, injecting bridge-mapped CWEs directly into context and preventing number-matching false positives (e.g., semantic retrieval returning CWE-203 for CAPEC-203 when the correct mapping is CAPEC-203 → CWE-15).
...
**Fixed components.** The single-model configuration uses Qwen2.5-7B-Instruct served via vLLM in FP16 with a 32,768-token context window on a single NVIDIA RTX 4090. The model is queried with a single chat completion call per CTI-Bench prompt at default temperature. The benchmark prompt is taken verbatim from CTI-Bench's `Prompt` column and is never modified, decomposed, or supplemented with additional reasoning steps. These choices ensure methodological comparability with the GPT-4 baseline.

**Knowledge base.** We construct a unified knowledge base of approximately 336,000 chunked documents from four public sources: the official MITRE CWE XML corpus (approximately 1,500 weakness entries with hierarchical structure, alternate terms, mitigations, and modes of introduction), the MITRE ATT&CK knowledge base (techniques, sub-techniques, groups, malware, tools, campaigns, and mitigations), the CAPEC corpus (attack patterns with cross-references to ATT&CK and CWE), and the National Vulnerability Database (CVE records from both CNA and ADP containers, yielding approximately 150,000 CVEs with structured CWE assignments). Cross-source relationships are extracted into explicit graph edges: ATT&CK technique → CAPEC attack pattern, CAPEC → CWE, and CVE → NVD-assigned CWE identifiers. For the paper-focused CTI-RCM evaluation, an RCM-only mode loads only CVE and CWE chunks, reducing prompt noise while preserving all load-bearing retrieval paths.

**Retrieval pipeline.** Each query goes through a multi-stage retrieval process:

1. **Hybrid retrieval:** BM25 sparse scoring and BGE-small dense embedding similarity with equal weight (α = 0.5), producing a top-K (K = 8) candidate pool.
2. **Graph expansion and bridge injection:** If the matching CVE chunk has NVD-assigned `cwe_ids`, those CWEs are injected as authoritative ground-truth context and competing CWE chunks are stripped. For ATT&CK/CAPEC queries, explicit cross-source bridge edges inject the correctly mapped entries.
3. **k-NN CWE voting:** For unmapped CVEs (no NVD CWE assignment), the query embedding's five nearest mapped CVE neighbors vote on candidate CWEs, weighted by cosine similarity. When the top vote's weighted share exceeds a confidence threshold (θ = 0.90) and its margin over the runner-up exceeds a confidence margin (1.50), the vote is treated as authoritative.
4. **Official CWE phrase selector:** When an `alternate_term` phrase from the MITRE CWE XML matches the query and the corresponding CWE is already present in retrieved context, the phrase selector promotes that CWE to the top of the candidate list.
5. **Cross-encoder reranking:** Each CWE candidate is jointly scored against the original query with `BAAI/bge-reranker-base` (278M parameters) in a full-attention pass, and the candidate ordering is updated.
6. **HyDE candidate expansion:** The LLM drafts a brief hypothetical CWE description from the CVE prose, the bi-encoder retrieves additional candidates against this hypothesis, and each new candidate is scored by the cross-encoder against the *original* CVE description. Candidates below a threshold (0.3) are discarded; up to five passing candidates are merged into the pool.
...
The system is organized as a single-process retrieval pipeline that loads all knowledge-base chunks, embeddings, and graph indexes at startup, then serves CTI-RCM queries by passing each query through the retrieval stages and into a deterministic model router. NVD-mapped CVEs are sent to Granite 4.1 8B, while NVD-unmapped CVEs are sent to Foundation-Sec-8B-Reasoning. The block diagram below summarizes the routed workflow.

![Inference pipeline for the routed ensemble. CVE description enters at the top and flows through hybrid retrieval, graph expansion, the mapped/unmapped fork, three CWE rerank stages, final context assembly, and runtime model routing: NVD-mapped cases go to Granite 4.1 8B, while NVD-unmapped cases go to Foundation-Sec-8B-Reasoning.](figures/pipeline_diagram.png){width=5in}
...
**Cross-source graph construction.** For every loaded chunk, the system shall extract authoritative cross-references (technique → CAPEC, CAPEC → CWE, CVE → NVD `cwe_ids`) and expose them as wrapper-keyed graph dictionaries usable for bridge injection at query time.
...
- <a id="ref-strom-2018"></a>Strom, B. E., Applebaum, A., Miller, D. P., et al. (2018). [MITRE ATT&CK: Design and philosophy](https://attack.mitre.org). *MITRE Technical Report*.
